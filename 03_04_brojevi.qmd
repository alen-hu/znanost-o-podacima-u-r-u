...
U kontekstu projekata analize podataka, brojčani podaci koriste se za mjerenje različitih varijabli od interesa, bilo da se radi o financijskim prihodima tvrtke, demografskim karakteristikama populacije ili performansnim metrikama modela strojnog učenja. Prema autorima knjige R for Data Science, numerički vektori su fundamentalni građevni blokovi analize podataka te se koriste u svim fazama projekta, od početnog istraživanja podataka do finalnog modeliranja. Ova svestranost numeričkih podataka čini ih nezamjenjivima u praktičnoj primjeni znanosti o podacima.

Važnost brojeva u znanosti o podacima očituje se kroz nekoliko ključnih dimenzija. Prvo, brojevi omogućuju precizno mjerenje i kvantifikaciju fenomena od interesa. Umjesto kvalitativnih opisa poput "visoka cijena" ili "niska prodaja", numerički podaci omogućuju precizne vrijednosti koje se mogu uspoređivati, kombinirati i analizirati matematičkim metodama. Drugo, brojevi služ kao osnova za primjenu statističkih testova i procedura koje omogućuju donošenje zaključaka temeljenih na dokazima umjesto na intuiciji ili pretpostavkama. Treće, numerički podaci omogućuju primjenu algoritama strojnog učenja koji otkrivaju složene obrasce i relacije u podacima koje bi ljudska analiza teško mogla detektirati.

Kada govorimo o Pokemon datasetu koji ćemo koristiti za ilustraciju koncepata, varijable poput "HP", "attack", "defense" i "speed" predstavljaju numeričke vrijednosti koje opisuju različite karakteristike Pokemon stvorenja. Ove brojčane vrijednosti omogućuju nam provedbu raznovrsnih analiza, od jednostavnih deskriptivnih statistika (prosječna snaga napada po generaciji) do složenijih prediktivnih modela (predviđanje je li Pokemon legendarni na temelju njegovih statističkih atributa).
...
numeric_weight <- parse_number(formatted_weight)
numeric_weight
```
:::

Slično tome, ako bi podaci o snazi napada u pokemon_dataset okviru bili zapisani s postotnim oznakama ili bodovima, `parse_number()` bi mogla izdvojiti čiste brojčane vrijednosti. Na primjer, ako bi varijabla attack sadržavala vrijednosti poput "Attack: 49 pts" ili "55%", funkcija bi automatski pretvorila te zapise u brojeve 49 i 55.
...
Funkcija `count()` broji koliko se puta svaka jedinstvena vrijednost pojavljuje u određenom stupcu podatkovnog okvira. Primjerice, ako želimo saznati koliko Pokémona pripada svakoj vrsti (type_1) u `pokemon_dataset` podatkovnom okviru, možemo jednostavno koristiti:

::: {.callout-note icon="false" collapse="true" title="Primjer"}
```{r}
pokemon_dataset |> count(type_1)
```
:::

Ovaj kod vraća tibble koji prikazuje svaku jedinstvenu vrstu Pokémona zajedno s brojem Pokémona koji pripadaju toj vrsti. Takav pristup omogućuje brzu identifikaciju najčešćih i najrjeđih vrsta u skupu podataka.

Unatoč savjetu iz ranijih poglavlja o stilskom formatiranju koda, funkcija `count()` se često stavlja u jedan redak jer se obično koristi u konzoli za brzu provjeru da li izračun funkcionira prema očekivanjima. Ova fleksibilnost u formatiranju odražava pragmatičan pristup analizi podataka gdje je brzina istraživanja često važnija od stroge adherencije stilskim pravilima.

Ako želimo vidjeti najčešće vrijednosti, možemo dodati argument `sort = TRUE` funkciji `count()`:

::: {.callout-note icon="false" collapse="true" title="Primjer"}
```{r}
pokemon_dataset |> count(type_1, sort = TRUE)
```
:::

Ova naredba daje popis vrsta Pokémona poredanih od najčešćih prema najrjeđima. Ovo je osobito korisno kada radimo s podatkovnim okvirima koji sadrže mnogo jedinstvenih vrijednosti i želimo brzo identificirati dominantne kategorije. U slučaju pokemon_dataset podatkovnog okvira, takva analiza otkriva da su Pokémoni vodenog tipa (water) najčešći, dok su neki specijalizirani tipovi znatno rjeđi.
