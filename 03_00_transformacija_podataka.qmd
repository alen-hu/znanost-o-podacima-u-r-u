...
Podaci su spremljeni u podatkovnom okviru pod nazivom `pokemon_dataset` koji sadrži 271 Pokemona i 13 varijabli:
...
**Transformacija podataka** (eng. *data transformation*) predstavlja fazu u radu s podacima koja se odnosi na sveobuhvatan proces sistematskog mijenjanja, prestrukturiranja i preoblikovanja sirovih podataka u oblik koji je prikladniji za analizu, modeliranje i donošenje odluka. Transformacija podataka je proces pripreme podataka za vizualizaciju i druge tehnike statističke interpretacije, a uključuje sposobnost statističkog razmišljanja i korištenja računalnih i algoritmičkih kapaciteta.

Transformacija označava skup tehnika i postupaka kojima se podatci organiziraju, mijenjaju i pripremaju za analizu. Riječ je o jednoj od najvažnijih faza u analitičkom procesu jer se stvarna korisnost podataka često otkriva tek nakon što se oni temeljito transformiraju, odnosno kada se strukturiraju i prikažu na način koji odgovara svrsi analize. Transformacija podataka, dakle, označava izmjenu strukture, sadržaja ili organizacije podatkovnog skupa bez nužnog mijenjanja samih vrijednosti varijabli.

Transformacija podataka predstavlja složen proces koji obuhvaća širok spektar tehnika i pristupa organiziranih u nekoliko glavnih kategorija:

**1. Pridruživanje i kombiniranje podataka (eng. *Data Joining*):** Analiza podataka rijetko se oslanja na jedan izvor podataka, nego je često potrebno raditi s više podatkovnih okvira u kojima su podaci sadržajni. Zato je jedan od temeljnih koraka u transformaciji kombiniranje bliskih podataka iz različitih izvora. Ovo uključuje različite vrste spajanja podatkovnih okvira u kojima su podaci sadržani, a koje omogućuju kombiniranje informacija na temelju zajedničkih ključeva.

**2. Manipulacija podataka (eng. *Data Manipulation*):** Manipulacija podataka obuhvaća osnovne operacije potrebne za upravljanje strukturom i sadržajem podatkovnih okvira. Paket `dplyr` predstavlja gramatiku za manipulaciju podataka pružajući konzistentan skup glagola koji pomažu u rješavanju najčešćih izazova manipulacije podataka. Osnovne operacije manipulacije podacima uključuju rad s redovima i stupcima kao što su filtriranje opažanja na temelju određenih uvjeta, odabir specifičnih stupaca, sortiranje podataka, dodavanje novih varijabli i druge.

**3. Nedostajuće vrijednosti (eng. *Missing Values*):** Nedostajuće vrijednosti predstavljaju čest izazov u podacima i zahtijevaju pažljiv pristup u transformaciji. Pritom rad s nedostajućim vrijednostima uključuje prepoznavanje različitih oblika nedostajućih podataka i razumijevanje uzroka njihova nastanka te pravilno postupanje s njima ovisno o prirodi podataka, uzroku njihovog javljanja i željenoj analizi.
